...
\textbf{Exercise 8.2}\par
    ...
    $(2)$We suppose every infinite subset $S \subseteq N_0^n$ contain an infinite chain, then when $n=N+1$, we consider the last element of the previous chain. There are two situations:\par
    $1.$The sequence of the last element is bounded.\par
    Obviously, at least one value (we call it x) appears infinite times in the sequence of the last element. We select those set with last element that equals to x, obviously it forms a infinite chain of $N_0^{n+1}$.\par

    $2.$The sequence of the last element is unbounded.\par
    Since each element is non-negative integer and the sequence is unbounded, we can find a sub-sequence of this sequence which is monotonically increasing. We select those set with last element in this sub-sequence, then it forms a infinite chain of $N_0^{n+1}$.\par
